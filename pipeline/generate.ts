...
// T2.2: shared with pipeline/errors.ts's post-hoc classifier (`classifyError`)
// so this pre-emptive "don't even try to parse it, surface immediately" check
// and run.ts's non-retryable-usage-limit classification can never drift apart.
import { isUsageLimitMessage } from './errors';
...
export async function generateLayout(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (isUsageLimitMessage(text)) throw new LlmError(`generation blocked by usage limit: ${text.slice(0, 120)}`);
    try {
