...
- **Every link resolves.** Buttons, menu items, and inline text links all need re-verification, because a converted button module can carry a broken or placeholder URL. Click through every link on the page, not just the primary CTA.
- **Forms submit and notify correctly.** Test-submit every converted form and confirm the notification email or webhook still fires. This is the single highest-cost QA miss on this whole list, because a broken contact form fails silently from the visitor's side.
