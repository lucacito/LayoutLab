...
In your WP admin sidebar, open the **AI Editor** menu, then click the **Settings** tab (the page opens on the Dashboard tab by default, and the connection panel lives under Settings). The connection panel shows three things you'll need:
...
- **No config file yet (or it's empty):** paste the snippet exactly as given, the whole thing with outer braces included, as the entire contents of `claude_desktop_config.json`. Save.
