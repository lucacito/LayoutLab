...
const PAGE: ThemePage = {
  role: 'landing2', roleLabel: 'Landing (new gen)',
  flow: [
...
async function main() {
  ...
  const { deps, close } = await buildThemeDeps({ businessName: BRIEF.businessName, logPrefix: '[brightwell2]', defaultMaxBudgetUsd: 4 });

  console.log(`[brightwell2] generating the Brightwell Family Dental fresh-clean family dentistry landing page…`);
  const result = await runThemePack(SPEC, deps);
  await close();
  console.log('[brightwell2] summary:', { generated: result.generated, ingested: result.ingested, dropped: result.dropped, deduped: result.deduped, slugs: result.pageSlugs });
}
